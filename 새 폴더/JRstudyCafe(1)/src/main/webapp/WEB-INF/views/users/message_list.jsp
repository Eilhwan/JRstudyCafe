...
<body>
	...
	<table>	
		<tr>
			<td colspan="4">
				...
				<button type="button" onclick='location.href="${conPath}/userMessagnerSendView.do"'>메시지작성</button>
			</td>
		</tr>
		<tr>
	...
	<c:if test="${not empty messages }">
		<c:forEach items="${messages }" var="message">
			<tr>
				<td><input type="checkbox" class="chkbox" value="${message.m_no }"></td>
				<c:if test="${message.m_status eq 2 }">
					<th><b><a href="${conPath }/messageDetail.do?m_no=${message.m_no}">${message.m_title }</a></b></th>
					<th>${message.m_sender }</th>
					<th>${message.m_sendtime }</th>
				</c:if>
				<c:if test="${message.m_status eq 1 }">
					<td><a href="${conPath }/messageDetail.do?m_no=${message.m_no}">${message.m_title }</a></td>
	...
	<div class="paging">
		<c:if test="${paging.startPage>paging.blockSize }">
			[ <a href="${conPath }/admin/MessangerList.do?pageNum=${paging.startPage-1}">이전</a> ]
		</c:if>
		...
		<c:if test="${paging.endPage<paging.pageCnt }">
			[ <a href="${conPath }/admin/MessangerList.do?pageNum=${paging.endPage+1}">다음</a> ]
		</c:if>
	</div>
</form>
</body>
