...
-- 리뷰게시판 좋아요 
SELECT * FROM RvLikes;
SELECT * FROM RvLikes WHERE U_ID='aaa' AND RV_NO=1;
DESC RvLikes;
...
INSERT INTO RvLikes (LIKENO, RV_NO, U_ID) 
    VALUES (RV_LIKENO_SQ.NEXTVAL, '1', 'aaa');
...
SELECT R.*, U.U_NICKNAME,U_IMAGE,U_NAME FROM RvComments R, USERS U WHERE R.U_ID= U.U_ID AND C_STATUS=1 AND RV_NO = 1 ORDER BY C_RDATE DESC;
...
INSERT INTO RvComments (C_NO,C_CONTENT,C_STATUS,RV_NO,U_ID) 
    VALUES (RV_CNO_SQ.NEXTVAL, '댓글내용', 1, 1, 'aaa' );
