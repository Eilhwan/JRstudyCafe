...
ul {
	...
	$( function() {
	   ...
	   $('#booking').click(function(){
			...
			var bk_stime = bk_date+'/'+stime;
			var bk_etime = bk_date+'/'+etime;
			var r_no = $('#r_no').val();
			if($("#r_no").val()==''){
				alert('방을 선택하세요');
				$("#r_no").css({
		   ...
		   if($("input[name=bk_date]").val()==''){
			...
			location.href='${conPath}/bookView.do?bk_date='+bk_date+'&bk_stime='+bk_stime+'&bk_etime='+bk_etime+'&r_no='+r_no;
	    });
		$('#r_no').change(function(){
...
<body>
	...
	<div class="book_step">
				<ul>
					...
					<li class="step_off"><b>STEP2</b> <br><span>입력내용 확인</span></li>
					<li class="step_arrow"> &gt; </li>
	...
	<table>
		<tbody>
				...
				<td>
					<select name="r_no" id="r_no">
						...
						<c:forEach items="${roomList}" var="room">
							<option value="${room.r_no }">${room.roomname}</option>
							<option value="r_no">1</option>
						    <option value="r_no">2</option>
						    <option value="r_no">3</option>
						</c:forEach>
					</select>
				</td>
			</tr>
			...
			<c:forEach items="${bookList }" var="book">
			  <c:if test="${bk_date eq bookList.bk_date }">
			<tr id="book_table_list">
				<td class="td1">
					예약 현황
				</td>
				<td>
					<b>${bookList.bk_stime } ~ ${bookList.bk_etime }</b> <div style="color: white; background-color: #ffb400; width: 20px; display: inline; font-size: 0.7em; border-radius: 2px; padding: 2px;" >예약불</div>
					<br>
					<b>10:00 ~ 13:00</b> <div style="color: white; background-color: #ffb400; width: 20px; display: inline; font-size: 0.7em; border-radius: 2px; padding: 2px;" >예약됨</div> 
					<br>
					<b>10:00 ~ 13:00</b> <div style="color: white; background-color: #ffb400; width: 20px; display: inline; font-size: 0.7em; border-radius: 2px; padding: 2px;" >사용중</div>
					<br>
					<b>10:00 ~ 13:00</b> <div style="color: white; background-color: #ffb400; width: 20px; display: inline; font-size: 0.7em; border-radius: 2px; padding: 2px;" >사용중</div> 
					<br>
	...
	<ul>
		...
		<div class="btn_list">
			...
			onclick="location.href='${conPath }/booking.do?bk_no='${bk_no}&r_no=${r_no}'"
			>
